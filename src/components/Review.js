...
const Review = (props) => {
  ...
  const retrieveKeywords = (id) => {
    ToothlessDataService.getKeyword(id)
      .then((response) => {
        let noDuplicates = response.data.keywords.reduce(function (
          previousValue,
          currentValue
        ) {
      ...
      {currentDocument ? (
        <div>
          ...
          <div className="d-flex justify-content-around">
            <Form onSubmit={handleSubmit} className="my-4">
              <Form.Group controlId="keywordCheckBox">
                ...
                <Form.Label as="article">
                  There are {keywords.length} unique named entities.
                </Form.Label>
                {keywords &&
                  keywords.map((keyword, index) => (
                    <div key={index} className="my-2 mx-2 d-flex">
                      <Form.Check
                        type="checkbox"
                        name="id[word]"
                        value={keyword}
              ...
              <label htmlFor="created-redaction-term">
                Add your own redaction term here:
              </label>
          ...
          <Link to={`/output/${props.match.params.id}`} className="nav-link">
            <Button
              type="submit"
              className="my-2 float-lg-right float-md-right float-sm-right btn-success"
            >
              To Output Page
            </Button>
          </Link>
